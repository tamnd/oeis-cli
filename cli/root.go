// Package cli builds the oeis command tree on top of the oeis library.
package cli
...
func Root() *cobra.Command {
	app := &App{cfg: oeis.DefaultConfig()}

	root := &cobra.Command{
		Use:   "oeis",
		Short: "Search the Online Encyclopedia of Integer Sequences",
		Long: `oeis searches the Online Encyclopedia of Integer Sequences (oeis.org)
through the public JSON search API. No authentication or API key is required.
It returns records as table, JSON, JSONL, CSV, TSV, or URLs.

oeis is an independent tool and is not affiliated with The OEIS Foundation.`,
		SilenceUsage:  true,
		SilenceErrors: true,
